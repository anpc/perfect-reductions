...
\begin{theorem}[Informal]\label{thm:introMain}
    ...
    Furthermore, the number of bits sent to the OT by the parties is $\tilde{O}\of{\poly\of{|\Y|}}$.\TODO{Verify.}
\end{theorem}
...
\begin{description}
    \item[Yao's Millionaires’ problem:] The server and the client each hold a number from 1 to $m$. The output is 1 if and only if the client's input is larger than the server's input. The matrix for this funcion has a constant column of 1's. After removing it, the last row of the matrix will the all 0 vector, and that the other rows are linearly independent. Hence it is full-dimensional.
...
\end{description}
\else
Yao's Millionaires’ problem is an example for such a function. The server and the client each hold a number from 1 to $m$. The output is 1 if and only if the client's input is larger than the server's input. The matrix for this funcion has a constant column of 1's. After removing it, the last row of the matrix will the all 0 vector, and that the other rows are linearly independent. Hence it is full-dimensional.
...
By the definition of $\bar{\p}$ it follows that $\bar{\p}^g+p_\bot\cdot\hat{\p}$ is a probability vector as well, hence \cref{eq:inconv} holds.
